...
### Narração, B-roll e prévia no link de aprovação
- **Narração** (seção própria no Estúdio, opcional): "Ouvir prévia" lê o texto com a voz sintética do navegador (só para ouvir: navegadores não deixam baixar essa voz); "Montar roteiro sem IA" / "Gerar roteiro de narração com IA" montam a fala de cada cena (mesma linha do tempo do vídeo) com tempo, tom e ritmo, para colar numa IA de voz externa (ElevenLabs, TTSMaker…) — cole **só a fala**; "Gravar minha voz" grava pelo microfone. O áudio anexado entra no "Gerar vídeo" desde a 1ª cena, com volumes separados de narração e música, e no editor avançado ("Adicionar narração", que nunca encurta o vídeo).
- **B-roll** (seção própria, opcional): busca vídeos e fotos gratuitos no Pexels e no Pixabay por `/api/broll` (as chaves ficam só no servidor; o download também passa pelo servidor, só dos domínios dos bancos). Clicar num resultado coloca o arquivo em "1. Materiais", ligado à cena escolhida. Chaves grátis: `PEXELS_API_KEY` (pexels.com/api) e `PIXABAY_API_KEY` (pixabay.com/api/docs). Sem nenhuma, a seção mostra como configurar.
- **Prévia no link de aprovação:** ao enviar a peça final (ou "Usar como peça final" no editor), o app gera nos bastidores uma versão reduzida (imagem: até 800 px, JPEG 70%; vídeo: 480p comprimido pelo ffmpeg.wasm) em `gcc/<cliente>/previews/<criativo>/`. O link de aprovação mostra **só a prévia** (com a legenda "qualidade reduzida"); o original fica intocado e fora do link. PDF e peças sem prévia seguem como antes. Publique as regras novas com `firebase deploy --only storage`.
