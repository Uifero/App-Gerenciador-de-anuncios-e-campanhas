...
export const view = (el, cliente) => montar(el, async (root, recarregar) => {
  ...
  const form = (p = {}) => `<form id="fp" class="space-y-3">
    ...
    <div><label class="label">Fotos</label><input type="file" name="fotos" accept="image/*" multiple class="block text-sm">
      ${(p.fotos || []).length ? `<div class="mt-2 flex flex-wrap gap-2">${p.fotos.map((f, i) => `<span class="relative"><img src="${esc(f.url)}" class="h-16 w-16 rounded object-cover" alt=""><button type="button" data-rmfoto="${i}" class="absolute -right-1 -top-1 rounded-full bg-rose-600 px-1 text-xs text-white">×</button></span>`).join('')}</div>` : ''}</div>
  ...
  const abrir = (p) => {
    ...
    on(m.el, 'submit', '#fp', async (f, ev) => {
      ...
      await ocupado(f.querySelector('[type=submit]'), async () => {
        ...
        for (const arq of novas) {
          const r = await enviarArquivo(`gcc/${cliente.id}/produtos/${id}/${Date.now()}_${arq.name.replace(/[^\w.-]/g, '_')}`, arq);
          fotos.push({ url: r.url, path: r.path });
        }
        await db.atualizar(COL.produtos, id, { ...dados, fotos });
        toast('Produto salvo.'); m.fechar(); recarregar();
      });
    });
    on(m.el, 'click', '[data-apagar]', async () => {
      ...
      await db.remover(COL.produtos, p.id); m.fechar(); recarregar(); toast('Produto apagado.');
    });
  };
  on(root, 'click', '[data-novo]', () => abrir(null));
  on(root, 'click', '[data-abrir]', (b) => abrir(produtos.find((x) => x.id === b.dataset.abrir)));
});
